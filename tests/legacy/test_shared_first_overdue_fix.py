...
DEPRECATED: These tests verify DATA_KID_OVERDUE_CHORES list tracking which was
removed in v0.5.0+. Overdue state is now tracked via DATA_KID_CHORE_DATA_STATE.
See test_chore_state_matrix.py and Phase 5 of CHORE_WORKFLOW_TESTING plan for
modern state-based tests.

Original purpose:
Validates that SHARED_FIRST chores correctly handle overdue states,
...
async def test_shared_first_pending_claim_becomes_overdue(
    ...
    with (
        ...
        with patch.object(coordinator, "_notify_overdue_chore", new=MagicMock()):
            ...
            chore_info = coordinator.chores_data[chore_id]
            assert chore_info[const.DATA_CHORE_STATE] == const.CHORE_STATE_CLAIMED, (
                f"Global state should be claimed (claim wins), "
                f"got: {chore_info[const.DATA_CHORE_STATE]}"
            )
