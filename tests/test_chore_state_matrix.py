...
class TestGlobalStateConsistency:
    ...
    @pytest.mark.asyncio
    async def test_independent_multi_kid_shows_independent_state(
        self,
        hass: HomeAssistant,
        scenario_shared: SetupResult,
    ) -> None:
        """Independent chore with multiple kids in different states shows INDEPENDENT global."""
        # Note: scenario_shared doesn't have independent chores with multiple kids
        # This would need a different scenario. For now, skip or use what we have.
        # Let's use a 2-kid shared_all chore and just verify state consistency.
        pytest.skip("Scenario with multi-kid independent chore not yet created")
